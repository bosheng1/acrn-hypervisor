...
static bool validate_plic_info(const struct plic_info *info)
{
	return (info->base != 0UL) && (info->size != 0UL) &&
		(info->source_num > 0U) && (info->context_num > 0U);
}

static void dump_plic_info(const struct plic_info *info, uint16_t vm_id)
{
	pr_info("Dump VM(%d) plic info", vm_id);
	pr_info("plic base: 0x%lx, size: 0x%lx", info->base, info->size);
	pr_info("plic max priority: %u", info->max_priority);
	pr_info("plic source number: %u", info->source_num);
	pr_info("plic context number: %u", info->context_num);
	for(uint32_t i = 0; i < info->context_num; i++) {
		pr_info("context id:%u, hartid:%u, parent_hwirq:%u",
			i, info->contexts[i].hartid, info->contexts[i].parent_hwirq);
	}
}

static int init_plic_info(struct acrn_vm *vm, struct plic_info *info)
{
	int ret = 0;
...
#else
	/* plic info will be generated by config tool when FDT parse is not enabled */ 
	struct acrn_vm_config *vm_config = get_vm_config(vm->vm_id);
	memcpy(info, vm_config->arch.plic, sizeof(struct plic_info));
#endif
	dump_plic_info(info, vm->vm_id);
	if (ret == 0)
		ret = validate_plic_info(info);
	return ret;
}
...
void vplic_init(struct acrn_vm *vm)
{
	...
	ret = init_plic_info(vm, &vplic->info);
	if (ret < 0) {
		pr_err("Failed to get vPLIC info for VM%d", vm->vm_id);
		return;
	}
