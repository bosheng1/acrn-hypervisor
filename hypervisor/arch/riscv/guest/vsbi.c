...
void init_vsbi(struct acrn_vm *vm)
{
	...
	for (i = 0; i < ARRAY_SIZE(vsbi_extensions); i++) {
		e = vsbi_extensions[i];
		 ...
		 */
		if (vm->arch_vm.n_vsbi_exts < MAX_NUM_SUPPORTED_VSBI_EXT) {
			if ((e->probe == NULL) || (e->probe(vm) == 0)) {
				vm->arch_vm.vsbi_exts[vm->arch_vm.n_vsbi_exts++] = e;
			}
		} else {
			pr_err("Failed to register vSBI extensions (ENOSPC), %s ignored", e->name);
		}
	}
}
