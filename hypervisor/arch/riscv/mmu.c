...
static struct mem_range rsvd_ranges[MAX_FDT_RSVD_RANGES];
static int nr_rsvd_ranges;
...
void init_phys_mem_range(void)
{
	...
	fdt_get_rsvd_mem_regions(get_host_fdt(), rsvd_ranges, &nr_rsvd_ranges);
#else
	...
	for (i = 0; i < nr_rsvd_ranges; i++) {
		pr_info("Reserved memory: [0x%lx-0x%lx]",
				rsvd_ranges[i].addr,
				rsvd_ranges[i].addr + rsvd_ranges[i].size);
	}
}
